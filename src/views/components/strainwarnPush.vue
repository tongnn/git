...
var zu='';
var eqCode='';//查询
var selsiteCode='';var siteName='';var siteId='' //根据字段=1过滤数据站点 
var seleqCode='';//站点编码过滤设备
...
function getQuerySql(type){
	...
	if(type=="0"){
		...
		if(selsiteCodes !=''){
			querySql = querySql + "and  EQ_TYPE = '1'";
		}
	}
	if(type=="1"){
		var seleqCodes=seleqCode;
		if(seleqCodes != ''){
			querySql = querySql + "and ID like '"+seleqCodes+"%' and  EQ_TYPE = '2' "
		}
	}
	if(type=='2'){
		var projectcode = eqCode;
		if(projectcode != ''){
			querySql = querySql + "and  EQ_CODE like '%" + projectcode + "%'";
		}
	}
	return querySql;
}
...
export default {
	...
	methods: {
			...
			saveSite(){
				...
				$("#DATA_POINT").val(siteName.EQ_NAME);
				$(".sitecodes").val(siteName.ID)
			...
			saveEq(){
				...
				$("#EQ_CODE").val(siteeqName.EQ_NAME);
				$(".siteeqs").val(siteeqName.ID)
    ...
    mounted(){
		...
		//选择数据站点
				...
				jQuery(grid_selector).jqGrid({
					...
					height: 100+'%',
				 	colNames:["ID",'测点编码','测点名称'],
                   	colNames:['监测设备编号',"监测设备名称",'站点过滤设备'],
					colModel:[
						{name:'ID',index:'ID', sorttype:"int", width:230,editable:true,editoptions:{edithidden:true}},
						{name:'EQ_NAME',index:'EQ_NAME',width:230,editable:true,editoptions:{size:"20",maxlength:"30"},editrules:{required:true}},
						{name:'NODE_NAME',index:'NODE_NAME',width:230,hidden:true, editable:true,editoptions:{size:"20",maxlength:"80"}}, //站点过滤设备
					], 
					viewrecords : true,
					rowNum: 10,
					rowList:[10,20,30],
			...
			//选择设备
			 jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid('navGrid','#grid-table_toppager',)
				$("#grid-eqtable_toppager,#grid-siteeqpager_left").hide();
				$("#grid-siteeqpager_center").css({width:'310px'})
				$("#grid-eqeqpager").hide();
			});	
		//本页
        jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					method:'get',
					url:baseURL + "/listDataController.htm?param_act=listData&param_funcId=0163af00c699000a",
					datatype: "json",
					height: 100+'%',
					...
					shrinkToFit:true,
					autoScroll:true,
					editurl: baseURL+"/manageCommon.htm?param_act=dataEdit&param_funcId=0163af00c699000a"
				});
				...
				function style_edit_form(form) {
					...
					$("#SEND_TYPE ").click(function(){
						if($("#SEND_TYPE option:selected").text() == '延时推送'){
							...
							$("#selsite,#selequip").css({left:'440px'});
						}else{
							$("#tr_CONTINUE_TIME").hide();
